...
from farkle import probability, max_score, enumerate_possible_dice_totals, scores_and_probs
...
def test_enumerate_possible_dice_totals(num_dice):
...
def test_scores_and_probs(num_dice, banked_score: int = 0):
    results = scores_and_probs(num_dice, banked_score)
    assert math.isclose(sum(results.values()), 1.0)